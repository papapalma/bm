...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  const [programsResult, traineesResult, sessionsResult] = await Promise.all([
    supabaseAdmin.from('programs').select('id, name, status'),
    supabaseAdmin.from('trainees').select('id, program_id, status'),
    supabaseAdmin.from('program_sessions').select('id, program_id'),
  ]);
  ...
  return successResponse({
    ...
    upcoming: programs.filter((program) => program.status === 'upcoming').length,
    completed: programs.filter((program) => program.status === 'completed').length,
    cancelled: programs.filter((program) => program.status === 'cancelled').length,
    totalTrainees: trainees.length,
    totalSessions: sessions.length,
    averageTraineesPerProgram: totalPrograms > 0 ? Number((trainees.length / totalPrograms).toFixed(2)) : 0,
    programStats,
  });
});
