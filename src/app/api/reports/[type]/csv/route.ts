...
export const GET = withErrorHandler(
  async (request: NextRequest, { params }: { params: Promise<{ type: string }> }) => {
    ...
    if (type === 'inventory') {
      const rows = (await itemService.getAllItems()).map((item) => ({
        id: item.id,
        name: item.name,
    ...
    if (type === 'lendings') {
      const rows = (await lendingService.getAllLendings({ start_date: startDate, end_date: endDate })).map((lending) => ({
        id: lending.id,
    ...
    if (type === 'trainees') {
      ...
      const rows = (await traineeService.getAllTrainees()).map((trainee) => ({
        id: trainee.id,
